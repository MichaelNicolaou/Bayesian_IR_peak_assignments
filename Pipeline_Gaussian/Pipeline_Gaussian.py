...
directory = 'Z:\\Michael Nicolaou\\Projects\\Back to the roots\\Calculation'
# path = "C:\\Users\\2811088N\\OneDrive - University of Glasgow\\Documents\\Calculated spectra"
path = "Z:\\Michael Nicolaou\\Projects\\Back to the roots\\Pipeline"
...
for filename in os.listdir(directory):
    ...
    for i in content:
        if "Frequencies --" in i:
            ...
            normal_toggle = 0
        # elif normal_toggle == 1 and "Normal Mode" not in i and "----" in content[content.index(i)-1]:
        #     print(i)
    ...
    #     if i != checkpoints[0]:
    #         for j in Normal_modes[]:
    #             print(j)
        # elif "Name" in i and normal_checkpoint == 1:
...
###############################################################################
#First graph - "Convolved" IR and Raman spectra
###############################################################################
...
###############################################################################
#Second graph - "Deconvolved" IR and Raman spectra
###############################################################################

#This attempts to ignore the effects of peak overlap by overwriting the overlap
#regions with the highest peak value, creating uniform peaks without the 
#overlapping gain in peak intensity.

#In order to not overwrite peaks and lose information, this is done in two 
#iterative "scans".

#The first "scan" draws the lower half of the distributions, starting from 0
#(from x-50 to x, where "x" indicates the frequency for the peak maximum),
#while scanning for already written values in each frequency. If another value
#is found, a comparison is made and the highest value is kept for that 
#frequency.

#The second "scan" then draws the higher half of the distributions 
#(from x to x+50), again scanning for already written values in each frequency
#and keeping the higher value.
...
###############################################################################
        ...
        for i in dfx:
            z = x.index(i)     
            if z > 50:
                xdata = np.array(x[(z - 50):(z + 1)])
            elif z < 50:
                xdata = np.array(x[0:(z + 1)])
            templist_IR.extend(list(xdata))
            
        for i in dfx:
            z = x.index(i)
            if IR_y[z] <= 100:
                c = 2
            elif IR_y[z] <= 250 and IR_y[z] > 100:
                c = 4
            else:
                c = 6  
            # xdata = np.array(x[(z - 50):z])
            if z > 50:
                xdata = np.array(x[(z - 50):(z + 1)])
            elif z < 50:
                xdata = np.array(x[0:(z + 1)])
            ...
            for k in xdata:
                if k in templist_IR:
                    z1 = x.index(k)
                    if gauss(k, IR_y[z], x[z], c) > IR_y[z1]:
                        IR_y[z1] = gauss((k), IR_y[z], x[z], c)
                        templist_IR.append(k)
        ...
        if Raman != []:            
            for i in dfx:
                ...
                if z > 50:
                    ...
                    xdata = np.array(x[0:(z + 1)])
                templist_Ram.extend(list(xdata))
                
            for i in dfx:
                z = x.index(i)
                if Ram_y[z] <= 100:
                    c = 2
                elif Ram_y[z] <= 250 and Ram_y[z] > 100:
                    c = 4
                else:
                    c = 6  
                ...
                for k in xdata:
                    if k in templist_Ram:
                        z1 = x.index(k)
                        if gauss(k, Ram_y[z], x[z], c) > Ram_y[z1]:
                            Ram_y[z1] = gauss((k), Ram_y[z], x[z], c)
                            templist_Ram.append(k)
                            counter_2 = counter_2 + 1
                        else:
                            continue
            
        print("Deconvolved spectra - First scan completed")
        ...
        for i in dfx:
            z = x.index(i)     
            if z > 3950:
                xdata = np.array(x[z:4000])
            elif z < 3950:
                xdata = np.array(x[z:(z + 51)])
            templist_IR.extend(list(xdata))
        ...
        print("Deconvolved spectra - Second scan completed")
